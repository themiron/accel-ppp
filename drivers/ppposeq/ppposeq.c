 ...
 * such as sstp. A pty is a byte stream: the tty flip buffer merges frames
 * written back to back (flush_to_ldisc hands receive_buf everything
 * committed since the last flush in one call), so PPP over a pty needs
 * HDLC framing to re-delimit frames. Here the socket is the ppp endpoint
 * and one datagram is one ppp frame, so no framing is done at all.
 *
...
/* poll returned unsigned int before __poll_t was introduced in 4.16 */
...
/* xmit_flags bits, as in ppp_synctty */
...
static int ppposeq_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	/*
	 * No txmunge here: keeping the queue step free of skb mutation means a
	 * full-queue retry (return 0) re-enters start_xmit on the same skb
	 * without re-munging it. The framing is applied exactly once, on the
	 * dequeue side, in ppposeq_recvmsg.
	 */
	...
	if (sock_queue_rcv_skb(sk, skb) < 0) {
		/*
		 * Receive queue full. Ask ppp_generic to retry: it requeues
		 * the skb, so do not free it here. ppposeq_recvmsg wakes us
		 * once userspace has drained something.
		 */
		return 0;
	}
			  ...
			  size_t total_len)
{
	...
	skb = sock_wmalloc(sk, NET_SKB_PAD + 2 + total_len, 0, GFP_KERNEL);
	if (!skb) {
		err = -ENOMEM;
		goto out;
	}
			  ...
			  size_t total_len, int flags)
{
	...
	/* add the negotiated framing, like ppp_sync_txmunge */
	if (likely(skb->len >= 2)) {
		p = skb->data;
		proto = (p[0] << 8) + p[1];

		/* LCP codes 1..7 must be sent uncompressed */
		islcp = (proto == PPP_LCP) && skb->len >= 3 && p[2] >= 1 && p[2] <= 7;

		/* compress protocol field if PFC is in effect */
		if ((ps->opt.flags & SC_COMP_PROT) && p[0] == 0 && !islcp)
			skb_pull(skb, 1);
...
out:
	...
	/* room freed: let ppp_generic push whatever it had queued */
 ...
 * family and protocol.
 */
static int ppposeq_connect(struct socket *sock, struct sockaddr *uservaddr,
			  int sockaddr_len, int flags)
{
	...
	/* reserve the address/control bytes ppposeq_recvmsg prepends, so the
